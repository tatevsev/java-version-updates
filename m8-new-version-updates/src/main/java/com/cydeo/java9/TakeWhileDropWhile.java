...
public class TakeWhileDropWhile {
    ...
    public static List<String> getStocksBelowFiveHundred(List<Stock> stocks){
        return stocks.stream()
                ...
                .takeWhile(TakeWhileDropWhile::isStockLessThanFiveHundred) //takeWhile works us limit() but if there is a condition, it is limiting based on Predicate condition, while limit() based on value. Takes all matching objects, in this case as we sorted our collection so the moment it reached the condition it will stop checking
                .map(Stock::getName)
                .collect(Collectors.toList());
    }
    ...
    public static List<String> getStocksAboveFiveHundred(List<Stock> stocks){
        return stocks.stream()
                ...
                .dropWhile(TakeWhileDropWhile::isStockLessThanFiveHundred) // works as skip all non matching but takes Predicate condition
                .map(Stock::getName)
                .collect(Collectors.toList());
    }
